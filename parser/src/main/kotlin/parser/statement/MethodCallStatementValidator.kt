...
class MethodCallStatementValidator : StatementTypeValidator{
  override fun isValid(statement: Statement): StatementValResult {
    ...
    if (statement.content[1].type != "PUNCTUATION" || statement.content[1].value != "(") {
      return StatementValResult(false, statement.content[1], "Expected an '(' after ${statement.content[0].value}, got '${statement.content[1].value}' instead")
    }
    ...
    if (statement.content[statement.content.size - 1].value != ";") {
      return StatementValResult(false, statement.content[statement.content.size - 1], "Expected a ';' at the end of the statement, got '${statement.content[statement.content.size - 1].value}' instead")
    }
