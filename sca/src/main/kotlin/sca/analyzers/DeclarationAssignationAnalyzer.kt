...
class DeclarationAssignationAnalyzer : StaticCodeAnalyzer {
    override fun analyzeNode(
        ...
        if (!checkIdentifierFormat(
                declarationAssignation.identifier,
                rules.identifierNamingCheck,
            )
        ) {
            columnIndex += 1
            issues.add(
                StaticCodeIssue(
                    "The identifier '${declarationAssignation.identifier}' must be in lower camel case.",
                    Position(lineIndex, columnIndex),
                ),
            )
        }
